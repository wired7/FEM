...
public:
																  ...
																  std::unordered_set<Geometry::HalfSimplex<simplexDim, GLuint>*>())
	{
		...
		auto neighbours = reinterpret_cast<HalfSimplexWithChildren<simplexDim, GLuint>*>(currentSimplex)->getAdjacentSimplices();
		std::set<std::pair<Geometry::HalfSimplex<simplexDim, GLuint>*, float>, HSPairCompare<simplexDim>> neighbourPairs;

		for (const auto& neighbour : neighbours)
		{
			auto n = reinterpret_cast<HalfSimplex<simplexDim, GLuint>*>(neighbour);
			embeddingDimVector directionToNeighborSimplex =
				normalize(getSimplexCentroid<simplexDim, embeddingDimVector>(n, positions) - simplexCentroid);
